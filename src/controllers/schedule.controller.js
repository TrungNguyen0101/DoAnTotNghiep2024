...
const findAll = async (req, res) => {
  let entities = await models.schedule.findAll({
    include: ["student"],
  });
...
const findById = async (req, res) => {
  let { id } = req.params;
  let entity = await models.schedule.findByPk(id, {
    include: ["student"],
  });
...
const create = async (req, res) => {
  let body = req.body;
  let entity = await models.schedule.create({
    schedule_id: uuidv4(),
    ...body,
  });
...
const update = async (req, res) => {
  let { id } = req.params;
  let body = req.body;
  let model = await models.schedule.findByPk(id);
  if (!model) {
...
const deleteById = async (req, res) => {
  let { id } = req.params;
  let result = await models.schedule.destroy({
    where: {
      schedule_id: id,
    },
  });
